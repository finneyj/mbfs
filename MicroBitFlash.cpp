...
uint32_t DEFAULT_SCRATCH_ADDR_buf[PAGE_SIZE];
uint32_t *DEFAULT_SCRATCH_ADDR;
...
{
	DEFAULT_SCRATCH_ADDR = (uint32_t *)((((uint32_t)DEFAULT_SCRATCH_ADDR_buf) & ~(PAGE_SIZE - 1)) + PAGE_SIZE);
}
...
// for O = 0 and N = 1: return 1 (need erase)
// for O = 1 and N = 1: return 1 (no need erase)
// for O = 1 and N = 0: return 1 (no need erase)
    ...
    uint8_t write_byte, int length, flash_mode m, uint8_t* scratch_addr)
{
    ...
    int erase = from_buffer == NULL ? 1 : need_erase(from_buffer, address, length);
